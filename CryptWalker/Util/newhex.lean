...
def hexStringToByteArrayAux : List Char → List UInt8 → Option (List UInt8)
  | [], acc => some acc.reverse
  | high :: low :: rest, acc =>
    match hexToByte high low with
    | some byte => hexStringToByteArrayAux rest (byte :: acc)
    | none => none
  | [_], _ => none
...
def hexStringToByteArray (s : String) : Option ByteArray :=
  if s.length % 2 ≠ 0 then
    ...
    match hexStringToByteArrayAux s.toList [] with
...
def falliableHexStringToByteArray (s : String) : ByteArray :=
  ...
  | 1, _ => match hexStringToByteArrayAux ("0" ++ s).toList [] with
        | some bytes => ByteArray.mk (Array.mk bytes)
        | none => panic! "Failed to convert hex string to ByteArray."
  | _, 0 => match hexStringToByteArrayAux s.toList [] with
        | some bytes => ByteArray.mk (Array.mk bytes)
        | none => panic! "Failed to convert hex string to ByteArray."
