...
partial def ByteArray.of_le32 (x: Subarray UInt32) (i: Nat := 0) (out: ByteArray := ByteArray.emptyWithCapacity (x.size * 4)): ByteArray
  := if h: i < x.size
...
partial def ByteArray.of_le64 (x: Subarray UInt64) (i: Nat := 0) (out: ByteArray := ByteArray.emptyWithCapacity (x.size * 8)): ByteArray :=
  if h: i < x.size then
    ByteArray.of_le64 x (i + 1) (out ++ UInt64.to_le x[i])
  else out

partial def ByteArray.of_be64 (x: Subarray UInt64) (i: Nat := 0) (out: ByteArray := ByteArray.emptyWithCapacity (x.size * 8)): ByteArray :=
  if h: i < x.size then
