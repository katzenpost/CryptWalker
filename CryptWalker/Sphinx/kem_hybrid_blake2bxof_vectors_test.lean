...
private def hybridKem := CryptWalker.KEM.kemMLKEM768X25519Blake2b
...
def main : IO UInt32 := do
  let path := "testdata/sphinx_kem_hybrid_blake2bxof_vectors.json"
  ...
  | .ok j =>
    ...
    | .ok arr =>
      ...
      | .error e => do IO.eprintln e; pure 1
      | .ok vecs =>
        IO.println s!"KEM-Sphinx hybrid (blake2b-xof) full-packet vectors ({vecs.size} from katzenpost)"
        let geomNoSurb := ofKEMWith "mlkem768-x25519-blake2bxof-kem" hybridKem 103 false 5
        let geomSurb := ofKEMWith "mlkem768-x25519-blake2bxof-kem" hybridKem 103 true 5
        let mut ok := true
        ...
        IO.println ""
        if ok then
          IO.println s!"all {vecs.size} KEM-Sphinx hybrid (blake2b-xof) vectors passed"
          pure 0
        else
          IO.eprintln "KEM-Sphinx hybrid (blake2b-xof) vectors FAILED"
          pure 1
